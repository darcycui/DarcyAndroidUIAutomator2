...
def wait_view_text_by_id2(device: u2.Device, view_id: str, timeout=120) -> str | None:
...
def wait_view_text_by_id(device: u2.Device, view_id: str, timeout=120) -> str | None:
...
def wait_view_text_by_text(device: u2.Device, text: str, timeout=120) -> str | None:
    print('等待元素出现:', text)
    target_view = get_view_by_text(device, text)
    b: bool = target_view.wait(timeout=timeout)
    return get_view_text_or_none(b, target_view)


def wait_view_text_by_text_contains(device: u2.Device, text: str, timeout=120) -> str | None:
    print('等待元素出现:', text)
