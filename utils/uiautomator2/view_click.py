...
def click_view_by_text(device: u2.Device, button_text: str):
    print('点击按钮:', button_text)
    delay(3)
    get_view_by_text(device, button_text).click()


def click_view_by_text_contains(device: u2.Device, button_text: str):
    print('点击按钮:', button_text)
    delay(3)
    get_view_by_text_contains(device, button_text).click()


def click_view_by_text_match(device: u2.Device, button_text: str):
    print('点击按钮:', button_text)
    delay(3)
    get_view_by_text_match(device, button_text).click()


def click_view_by_class_name(device: u2.Device, class_name: str):
    print('点击按钮:', class_name)
    delay(3)
    get_view_by_class_name(device, class_name).click()


def click_view_by_id(device: u2.Device, button_id: str):
